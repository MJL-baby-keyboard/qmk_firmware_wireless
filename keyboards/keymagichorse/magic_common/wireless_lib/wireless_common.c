...
# if defined(WIRELESS_BATTERY_ENABLE)	// 按键按下时 最好不要老是定时发电量数据
    battery_reset_timer();
#endif
...
#   if defined(WIRELESS_LPM_ENABLE)
    ...
    // 蓝牙模式点按
    if(keycode == BL_SW0 || keycode == BL_SW1 || keycode == BL_SW2)
    {
...
//  切换主机任务
void wireless_switch_host_task(void){
    static bool has_switched = false;
    // 蓝牙模式长按的
    if (this_down_wireless_keycode == BL_SW0 ||
        this_down_wireless_keycode == BL_SW1 ||
        this_down_wireless_keycode == BL_SW2)
    {
...
bool via_command_wireless(uint8_t *data, uint8_t length)
{
